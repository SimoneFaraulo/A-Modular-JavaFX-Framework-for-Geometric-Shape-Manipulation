...
/**
 * Integration test class for the {@link ForegroundCommand} functionality.
 * This test verifies that a shape can be brought to the front of the drawing canvas.
 */
@ExtendWith(ApplicationExtension.class)
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class ForegroundCommandTest {
    ...
    /**
     * Tests the {@link ForegroundCommand} by:
     * <ul>
     *   <li>Adding two shapes to the canvas</li>
     *   <li>Ensuring the second one is initially on top</li>
     *   <li>Using ForegroundCommand to bring the first shape to front</li>
     *   <li>Verifying the first shape is now on top</li>
     * </ul>
     *
     * @param robot the TestFX robot
     */
    @Test
    void testBringShapeToFront(FxRobot robot) {
        ...
        RectangleShape shape2 = new RectangleShape(Color.RED, Color.LIGHTCORAL, 100, 100);

        robot.interact(() -> {
            canvas.getChildren().addAll(shape1, shape2);
        });
        assertThat(canvas.getChildren().get(canvas.getChildren().size() - 1)).isEqualTo(shape2);
        ForegroundCommand command = new ForegroundCommand();
        command.setSelectedShape(shape1);
        robot.interact(command::execute);
        assertThat(canvas.getChildren().get(canvas.getChildren().size() - 1)).isEqualTo(shape1);
    }
}
